            ...
            let responseStatus = response as! HTTPURLResponse
             if responseStatus.statusCode == 200{
                ...
                if error == nil, let responseResult = (try? JSONSerialization.jsonObject(with: data!, options: [.mutableContainers])) {
                    ...
                    if let dataArray = responseResult as? NSDictionary
                    {
                        print("dataArray =\(dataArray)")
                        print("Get page \(String(describing: dataArray["page"]as? Int))")
                        print("Get per_page \(String(describing: dataArray["per_page"]as? Int))")
                        print("Get total \(String(describing: dataArray["total"]as? Int))")
                        print("Get total_pages \(String(describing: dataArray["total_pages"]as? Int))")
